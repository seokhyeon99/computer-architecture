...
struct cacheline D_cache[128] = {0,};
struct seper_inst s;
...
int ReadMem(int addr){
	unsigned int tag    = (addr & 0xffffe000)>>13;
        unsigned int index  = (addr & 0x00001fc0)>>6;
        unsigned int offset = (addr & 0x0000003f);
	unsigned int dest   = (addr & 0xffffffc0);
        if(D_cache[index].tag == tag && D_cache[index].valid == 1){//Hit
		h_count++;
		//printf("------------Hit-----------\n");
		return D_cache[index].data[offset/4];
	}
	else{//Miss
		ms_count++;
		//printf("------------Miss-----------\n"); 
		D_cache[index].tag = tag;
		D_cache[index].valid = 1;
		for(int i=0; i<16; i++){
			D_cache[index].data[i] = Memory[dest/4+i];
		}
		return D_cache[index].data[offset/4];
	}
}
void WriteMem(int addr, int value){
	...
	unsigned int tag    = (addr & 0xffffe000)>>13;
        ...
        unsigned int offset = (addr & 0x0000003f);
	unsigned int dest   = (addr & 0xffffffc0);
	if(D_cache[index].tag == tag && D_cache[index].valid == 1){
		D_cache[index].data[offset/4] = value;
		Memory[addr/4] = value;
	}
	else{
		D_cache[index].tag = tag;
...
void memory_access(){
        ...
        if(cs.MemWrite){ // sw
                ...
                WriteMem(s.alu_result, Reg[s.rt]);
		M_count++;
        }
        if(cs.MemRead){ // lw
                //read_data = Memory[s.alu_result/4];
                read_data = ReadMem(s.alu_result);
		M_count++;
        }
...
void print_result(){
        ...
        printf("|\t\tNumber of total branches = %d\t\t\t|\n", B_total_count);
        printf("|\t\tNumber of Hit = %d\t\t\t\t|\n", h_count);
        printf("|\t\tNumber of Miss = %d\t\t\t\t|\n", ms_count);
        printf("|\t\tNumber of register operation = %d\t\t|\n", Reg_count);
